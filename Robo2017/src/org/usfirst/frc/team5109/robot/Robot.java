 ...
 * Robot setup:
 *
 * Controls: Automatic gears approach: left trigger
 * 
 * 
 * 
 * 
...
public class Robot extends SampleRobot {
	 ...
	 * Notes:
	 * 
	 * To declare a motor run by a talon if the talon is controlled by CAN you
	 * must: import com.ctre.CANTalon declare each motor by doing CANTalon
	 * motorName;
	 */
	...
	// these constants should be used every time drive motors called - they
	// change motor direction if wiring backwards
	// use by doing constant*value within the argument (the ()) for the set
	// method (.set) for each motor
	...
	/*
	 * AnalogOutput cameraAim; Victor cameraAim2;
	 */
	...
	public void turnDegrees(double degrees) {
		...
		if (degrees > 0) {
			while (rightDrivetrain.getDistance() * rightEncoderConstant < distance
					&& leftDrivetrain.getDistance() * leftEncoderConstant > -distance
					&& !operatorStick.getRawButton(11)) {
				DriverStation.reportError("Left Drivetrain: " + leftDrivetrain.getDistance(), false);
				DriverStation.reportError("Right Drivetrain: " + rightDrivetrain.getDistance(), false);
	...
	public void moveForward(double distance) {
		...
		while (rightDrivetrain.getDistance() * rightEncoderConstant < distance
				&& leftDrivetrain.getDistance() * leftEncoderConstant < distance && !operatorStick.getRawButton(11)) {
	...
	public void moveBackward(double distance) {
		...
		rightDrivetrain.reset();
		leftDrivetrain.reset();
		// since input is a positive distance, it is made negative then encoders
		// count down to it
		while (rightDrivetrain.getDistance() * rightEncoderConstant > distance
				|| leftDrivetrain.getDistance() * leftEncoderConstant > distance && !operatorStick.getRawButton(11)) {
			rightDrive1.set(rightMoveSpeed * rightDriveConstant);
	...
	public void autoGearNew() { // 2017GRITS auto gear with vision tracking
		...
		while (keepDriving && !rightStick.getRawButton(12)) {
			...
			turnDegrees(angle);// (xPosition / (cameraPxWidth/2)) *
								// (cameraViewingAngle/2));
			Timer.delay(.5);
		}
		pushGear();
	...
	public String getVisionData() { // 2017GRITS new command, reads file with
									// vision data
	...
	public void autoDoGear() {
		...
		if (direction == -1) {
			// if it is to the left we turn right (this value is added to the
			// right motor)
			changeSpeed = .1;
		} else if (direction == 1) {
			changeSpeed = -.1;
	...
	public void autonomous() {
		...
		double waitTime = .01;
		/*
		 * 1 - Red Starting Position Left 2 - Red Starting Position Middle 3 -
		 * Red Starting Position Right 4 - Blue Starting Position Left 5 - Blue
		 * Starting Position Middle 6 - Blue Starting Position Right
		 */
		switch (autoMode) {
		case 1:
		...
		case 2:
			...
			/*
			 * turnDegrees(-60); leftDrive1.set(1 * leftDriveConstant);
			 * leftDrive2.set(1 * leftDriveConstant); rightDrive1.set(.95 *
			 * rightDriveConstant); rightDrive2.set(.95 * rightDriveConstant);
			 * Timer.delay(2.5); shooter1.set(shooter1Constant * shooterSpeed);
			 * shooter2.set(shooter2Constant * shooterSpeed); Timer.delay(.5);
			 * leftDrive1.set(0); leftDrive2.set(0); rightDrive1.set(-.95 *
			 * rightDriveConstant); rightDrive2.set(-.95 * rightDriveConstant);
			 * Timer.delay(.2); rightDrive1.set(0); rightDrive2.set(0);
			 * intake.set(intakeConstant * intakeSpeed);
			 * 
			 * while(isAutonomous()) { shoot(); } intake.set(0);
			 */
			break;
		case 3:
			// moves forward to cross baseline/get close to peg
	...
	public void hitHopperThenShootRed() {
		// hit hopper (adjust distance using the Timer.delay - make sure robot
		// squares itself up against the hopper so push longer than needed)
		...
		intake.set(intakeConstant * intakeSpeed);
		Timer.delay(.05);
		// stops the motors to wait for balls to fall in, use Timer.delay to
	...
	public void hitHopperThenShootBlue() {
		...
		// this will last till end of auto - during teleop once enabled shooter
		// should reset to 0 on its own
	}
	 ...
	 * 
	 * Ouputs: String array with [double, int] - distance in meters, direction
	 * (left/right/center) for int: -3 - no connection -2 - no goal seen -1 -
	 * left 0 - straight 1 - right
	 */
	public String[] getHighShot() {
		try {
			...
			DriverStation.reportError(
					"This is damn near impressive. Successfully connected to socket and retrieved output.", true);
	...
	public String[] getLaser() {
		try {
			...
			String str = br.readLine();
			DriverStation.reportError(
					"This is damn near impressive. Successfully connected to socket and retrieved output.", false);
	...
	/*
	 * 
	 * Methods that have been completed: doGear doHighShot
	 * 
	 * Methods that have not been completed: Movement tracking doHopper
	 * doLoadingStation
	 */
	public void operatorControl() {
		...
		while (isOperatorControl() && isEnabled()) {
			...
			/*
			 * DriverStation.reportError("Left Drive 1 " +
			 * leftDrive1.getOutputCurrent(), false);
			 * DriverStation.reportError("Left Drive 2 " +
			 * leftDrive2.getOutputCurrent(), false);
			 * DriverStation.reportError("Right Drive 1 " +
			 * rightDrive1.getOutputCurrent(), false);
			 * DriverStation.reportError("Right Drive 2 " +
			 * rightDrive2.getOutputCurrent(), false);
			 * DriverStation.reportError("Intake " + intake.getOutputCurrent(),
			 * false); DriverStation.reportError("Elevator " +
			 * elevator.getOutputCurrent(), false);
			 * DriverStation.reportError("Scale 1 " + scale1.getOutputCurrent(),
			 * false); DriverStation.reportError("Scale 2 " +
			 * scale2.getOutputCurrent(), false);
			 * DriverStation.reportError("Shooter 1 " +
			 * shooter1.getOutputCurrent(), false);
			 * DriverStation.reportError("Shooter 2 " +
			 * shooter2.getOutputCurrent(), false);
			 */
	...
	public void autoGear() {
		/*
		 * while(interpretUltrasonicSensor() > .25 && Timer.getMatchTime() <
		 * 15.0) { autoDoGear(); } rightDrive1.set(0); rightDrive2.set(0);
		 * leftDrive1.set(0); leftDrive1.set(0); if (Timer.getMatchTime() >=
		 * 15.0) { return; } else { pushGear(); } moveBackward(30);
		 */
	...
	public void oldAuto() {
		...
		Timer.delay(1.9);
		// if we are blue, left needs to stop, if we are red, right needs to
		// stop
		// rightDrive1.set(0);
		// rightDrive2.set(0);
		leftDrive1.set(0);
		...
		leftDrive2.set(0);
		/*
		 * Baseline then shooter rightDrive1.set(-.75 * rightDriveConstant);
		 * rightDrive2.set(-.75 *rightDriveConstant); leftDrive1.set(-.77 *
		 * leftDriveConstant); leftDrive2.set(-.77 * leftDriveConstant); //2.1
		 * for middle Timer.delay(2.3); rightDrive1.set(0); rightDrive2.set(0);
		 * leftDrive1.set(0); leftDrive2.set(0); Timer.delay(.2);
		 * rightDrive1.set(.75 * rightDriveConstant); rightDrive2.set(.75
		 * *rightDriveConstant); leftDrive1.set(.77 * leftDriveConstant);
		 * leftDrive2.set(.77 * leftDriveConstant); Timer.delay(1.3);
		 * rightDrive1.set(.77 * rightDriveConstant); rightDrive2.set(.77 *
		 * rightDriveConstant); leftDrive1.set(-.77 * leftDriveConstant*0);
		 * leftDrive2.set(-.77 * leftDriveConstant*0); Timer.delay(.9);
		 * rightDrive1.set(0); rightDrive2.set(0); leftDrive1.set(0);
		 * leftDrive2.set(0); Timer.delay(.2); rightDrive1.set(.75 *
		 * rightDriveConstant); rightDrive2.set(.75 *rightDriveConstant);
		 * leftDrive1.set(.77 * leftDriveConstant); leftDrive2.set(.77 *
		 * leftDriveConstant); Timer.delay(1.5); rightDrive1.set(0);
		 * rightDrive2.set(0); leftDrive1.set(0); leftDrive2.set(0);
		 * shooter1.set(shooter1Constant * shooterSpeed);
		 * shooter2.set(shooter2Constant * shooterSpeed);
		 * elevator.set(elevatorConstant * elevatorSpeed);
		 * intake.set(intakeSpeed * intakeConstant); Timer.delay(6);
		 */

		/*
		 * Center Gear then shooter rightDrive1.set(-.75 * rightDriveConstant);
		 * rightDrive2.set(-.75 *rightDriveConstant); leftDrive1.set(-.77 *
		 * leftDriveConstant); leftDrive2.set(-.77 * leftDriveConstant); //2.1
		 * for middle Timer.delay(1.95); rightDrive1.set(0); rightDrive2.set(0);
		 * leftDrive1.set(0); leftDrive2.set(0); pushGear(); Timer.delay(.3);
		 * rightDrive1.set(.75 * rightDriveConstant); rightDrive2.set(.75
		 * *rightDriveConstant); leftDrive1.set(.77 * leftDriveConstant);
		 * leftDrive2.set(.77 * leftDriveConstant); Timer.delay(1.8); //red
		 * //leftDrive1.set(-.77 * leftDriveConstant); //leftDrive2.set(-.77 *
		 * leftDriveConstant); //blue rightDrive1.set(-.77 *
		 * rightDriveConstant); rightDrive2.set(-.77 * rightDriveConstant);
		 * Timer.delay(.35); rightDrive1.set(-.75 * rightDriveConstant);
		 * rightDrive2.set(-.75 *rightDriveConstant); leftDrive1.set(-.77 *
		 * leftDriveConstant); leftDrive2.set(-.77 * leftDriveConstant);
		 * Timer.delay(3.0); rightDrive1.set(0); rightDrive2.set(0);
		 * leftDrive1.set(0); leftDrive2.set(0); shooter1.set(shooter1Constant *
		 * shooterSpeed); shooter2.set(shooter2Constant * shooterSpeed);
		 * elevator.set(elevatorConstant * elevatorSpeed);
		 * intake.set(intakeSpeed * intakeConstant); Timer.delay(6);
		 */
	}
	...
	/*
	 * Old Teleop Controls (for just 1 driver) leftDrive1.set(leftDriveConstant
	 * * leftStick.getY()); leftDrive2.set(leftDriveConstant *
	 * leftStick.getY()); rightDrive1.set(rightDriveConstant *
	 * rightStick.getY()); rightDrive2.set(rightDriveConstant *
	 * rightStick.getY());
	 * 
	 * if (rightStick.getRawButton(2)) { if (lowGear) { leftShift.set(true);
	 * rightShift.set(true); lowGear = false; } else { leftShift.set(false);
	 * rightShift.set(false); lowGear = true; } }
	 * 
	 * if (rightStick.getRawButton(5)) { shooterSpeed += .01; Timer.delay(.1); }
	 * if (rightStick.getRawButton(6)) { shooterSpeed -= .01; Timer.delay(.1); }
	 * 
	 * //right trigger shoots the ball if (rightStick.getTrigger()) {
	 * shooter1.set(shooter1Constant * shooterSpeed);
	 * shooter2.set(shooter2Constant * shooterSpeed);
	 * elevator.set(elevatorConstant * -1 * elevatorSpeed);
	 * intake.set(intakeConstant * intakeSpeed); } else { shooter1.set(0);
	 * shooter2.set(0); elevator.set(0); }
	 * 
	 * 
	 * //left trigger punches out the gear if (leftStick.getTrigger()) {
	 * pushGear(); }
	 * 
	 * //left stick button 3 does gear flap if (leftStick.getRawButton(3)) {
	 * flap.set(false); } else { flap.set(true); }
	 * 
	 * //right stick button 3 toggles intake if (rightStick.getRawButton(3)) {
	 * if (intakeBool) { intakeBool = false; } else { intakeBool = true; }
	 * Timer.delay(.2); }
	 * 
	 * //intake if (intakeBool ) {+ if (!rightStick.getTrigger()) {
	 * intake.set(intakeSpeed * intakeConstant); } } else { if
	 * (!rightStick.getTrigger()) { intake.set(0); } }
	 * 
	 * //right stick button 4 starts scaling if (rightStick.getRawButton(4)) {
	 * 
	 * //scale1.set(.5 * scale1Constant); scale2.set(.5 *scale2Constant); } else
	 * { scale1.set(0); scale2.set(0); }
	 */
	...
	public double interpretUltrasonicSensor() {
		/*
		 * For users of this output that desire to work in voltage, a 5V power
		 * supply yields~4.88mV per 5 mm. Output voltage range when powered with
		 * 5V is 293mV for 300-mm, and 4.885V for 5000-mm.
		 */
	...
	public void test() {
		/*
		 * boolean temp = true; while(temp){ Timer.delay(1.1); turnDegrees(1); }
		 */
	}
	/*
	 * int scaleTest = 0; boolean startup = false; while (isTest() &&
	 * isEnabled()) { if (leftStick.getRawButton(11)) { doGear(); } if
	 * (leftStick.getRawButton(2)) { String[] output = getData();
	 * 
	 * DriverStation.reportError("Ultrasonic Sensor: " +
	 * interpretUltrasonicSensor(), false);
	 * 
	 ...
	 * DriverStation.reportError("Scaler 1 Current: " +
	 * scale1.getOutputCurrent(), false);
	 * DriverStation.reportError("Scaler 2 Current: " +
	 * scale2.getOutputCurrent(), false); } if (rightStick.getRawButton(4)) {
	 * DriverStation.reportError("Right encoder" +
	 * rightDrivetrain.getDistance(), false);
	 * DriverStation.reportError("Left Encoder" + leftDrivetrain.getDistance(),
	 * false); } if (leftStick.getRawButton(7)) { moveBackward(20); } if
	 * (leftStick.getRawButton(8)) { scale2.set(scale2Constant * .7);
	 * scale1.set(scale1Constant * .7); } else { scale2.set(0); scale1.set(0); }
	 * if (leftStick.getRawButton(9)) {
	 * DriverStation.reportError("Left Drivetrain: " +
	 * leftDrivetrain.getDistance(), false);
	 * DriverStation.reportError("Right Drivetrain: " +
	 * rightDrivetrain.getDistance(), false); turnDegrees(360);
	 * DriverStation.reportError("Left Drivetrain: " +
	 * leftDrivetrain.getDistance(), false);
	 * DriverStation.reportError("Right Drivetrain: " +
	 * rightDrivetrain.getDistance(), false); } if (leftStick.getRawButton(10))
	 * { DriverStation.reportError("Left Drivetrain: " +
	 * leftDrivetrain.getDistance(), false);
	 * DriverStation.reportError("Right Drivetrain: " +
	 * rightDrivetrain.getDistance(), false); moveForward(30);
	 * DriverStation.reportError("Left Drivetrain: " +
	 * leftDrivetrain.getDistance(), false);
	 * DriverStation.reportError("Right Drivetrain: " +
	 * rightDrivetrain.getDistance(), false); }/* /* if (leftStick.getTrigger())
	 * { shooter1.set(shooter1Constant * (shooterSpeed + .075));
	 * shooter2.set(shooter2Constant * (shooterSpeed + .075));
	 * DriverStation.reportError("Shooter 1 : " + shooter1.getOutputCurrent(),
	 * false); DriverStation.reportError("Shooter 2 : " +
	 * shooter2.getOutputCurrent(), false);
	 ...
	 * 
	 * if (!startup) { shooter1.set(shooter1Constant * shooterSpeed);
	 * shooter2.set(shooter2Constant * shooterSpeed); Timer.delay(2); startup =
	 * true; } elevator.set(elevatorConstant * elevatorSpeed); } else {
	 * shooter1.set(0); shooter2.set(0); elevator.set(0); }
	 */

	/*
	 * if (rightStick.getRawButton(3)){
	 * 
	 * elevator.set(elevatorConstant * elevatorSpeed); //might be wrong look
	 * here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 * ! (try : elevatorSpeed * elevatorConstant) Timer.delay(.2); } if
	 * (rightStick.getRawButton(5)) { if (scaleTest < 3) { scaleTest++; } else {
	 * scaleTest = 0; } Timer.delay(.2); } if (rightStick.getRawButton(12) &&
	 * !leftStick.getTrigger()) { shoot(); } else { shooter1.set(0);
	 * shooter2.set(0); elevator.set(0); }
	 */

	/*
	 * if (scaleTest == 1) { scale1.set(.3 * scale1Constant); scale2.set(.3 *
	 * scale2Constant); } else if (scaleTest == 2) { scale1.set(0);
	 * scale2.set(0); } else if (scaleTest == 3) { scale1.set(.7 *
	 * scale1Constant); scale2.set(.7 * scale2Constant); } else { scale1.set(0);
	 * scale2.set(0); }
	 */

	/*
	 * if (rightStick.getTrigger()) { pushGear(); } if
	 * (rightStick.getRawButton(6)) {
	 * 
	 * shooterSpeed += .01; DriverStation.reportError("Shooter Speed:" +
	 * shooterSpeed, false); Timer.delay(.2); } if (rightStick.getRawButton(5))
	 * {
	 * 
	 * shooterSpeed -= .01; DriverStation.reportError("Shooter Speed:" +
	 * shooterSpeed, false); Timer.delay(.2); } if (leftStick.getRawButton(3)) {
	 * flap.set(false); } else { flap.set(true); } if
	 * (rightStick.getRawButton(2)) { if (lowGear) { leftShift.set(true);
	 * rightShift.set(true); lowGear = false; } else { leftShift.set(false);
	 * rightShift.set(false); lowGear = true; } Timer.delay(.2); } if
	 * (leftStick.getRawButton(5)) { intake.set(intakeConstant * intakeSpeed); }
	 * else { intake.set(0); }
	 * 
	 * rightDrive1.set(rightDriveConstant * rightStick.getY());
	 * rightDrive2.set(rightDriveConstant * rightStick.getY());
	 * leftDrive1.set(leftDriveConstant * leftStick.getY());
	 * leftDrive2.set(leftDriveConstant * leftStick.getY()); Timer.delay(.005);
	 * 
	 * } }
	 */
}
