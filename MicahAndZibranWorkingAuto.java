...
public class Robot extends IterativeRobot {
	...
	*/
	...
	public void autonomousPeriodic() {
		int startpos = 1;// 1 is right side, 2 is right middle, 3 is left middle, 4 is left side
	...
	//compressor = new Compressor(0);
		...
		double imuYaw = (imuTable.getEntry("yaw").getDouble(0));
		double imuPitch = (imuTable.getEntry("pitch").getDouble(0));
		double imuRoll = (imuTable.getEntry("roll").getDouble(0));
		...
		if (imuTable.getEntry("roll").getDouble(0) < 135 && imuTable.getEntry("roll").getDouble(0) > 0) {
			while (imuTable.getEntry("roll").getDouble(0) < 135 && imuTable.getEntry("roll").getDouble(0) > 0) {
		...
		if (imuTable.getEntry("roll").getDouble(0) > -145 && imuTable.getEntry("roll").getDouble(0) < 0) {
			while (imuTable.getEntry("roll").getDouble(0) > -145 && imuTable.getEntry("roll").getDouble(0) < 0) {
				leftMotor1.set(ControlMode.PercentOutput, .25);
				leftMotor2.set(ControlMode.PercentOutput, .25);
				rightMotor1.set(ControlMode.PercentOutput, -.25);
				rightMotor2.set(ControlMode.PercentOutput, -.25);				
